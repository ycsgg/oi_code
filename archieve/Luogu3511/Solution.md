...
二分之后先把大于的边全删掉，没用，这样我们就得到了一个混合图

混合图找欧拉回路就需要把无向边定向，每次操作可以使某一点入度 $+1$ ，出度 $-1$，目标是出度=入度

显然这两个奇偶性得相同，不然没得搞

如果我们钦定一些边的方向，就可以在此基础上调整

借助网络流的模型，无向边之间先定向为 $(u,v)$ 然后连一条 $(v,u,1)$ 的边，若入度大于出度，需要流入 入度-出度 的流量才能调整

反之同理，如果流满了就可行

找路径沿着没走过的出边向下走，回溯加到一个数组即可
