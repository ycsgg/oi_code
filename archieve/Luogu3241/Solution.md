...
先不考虑$[l,r]$的限制，求点$p$到所有点的距离，枚举```lca```算贡献， 是$O(n)$的，在点分树上就是$O(\log n)$

需要处理的信息有每个点所有的祖先和到所有祖先的距离，因为点分树没啥好的原树的性质，只能暴力存

然后另开一个```vector```存所有子树的点到自己的距离

在点分树上暴力扫$p$的祖先，对于每个祖先$p'$,$ans+=dis(p,p')+dis(p,p')\times p'中不含p的子树siz+p'中不含p的点到p'距离和$

不含$p$完全可以大力扫，因为度数不会很多

点权的限制考虑可以继续暴力，每个点上挂三个```vector```每个```vector```存这个子树里所有的点的点权与深度，因为保证度数和点分树所以空间复杂度完全正确$O(n\log n)$
