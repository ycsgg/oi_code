...
虽然有更妙的欧拉序+线段树的做法，但是DDP它不用动脑子啊（~~智障选手的福音~~

设 $dp_{x,0}$ 表示 $x$ 的子树内直径长度，$dp_{x,1}$ 表示起点为 $x$ 的最长链

$dp_{x,0}=\max\{\max\limits_{y\in son(x)}dp_{y,0},\max\limits_{y,z \in son(x)}dp_{y,1}+dp_{z,1}\}$

$dp_{x,1}=\max\limits_{y\in son(x)}\{dp_{y,1}\}+w(x)$

DDP套路，设

$g_{x,1}=\max\limits_{y\in light(x)}dp_{y,1}$
$g_{x,0}=\max\{\max\limits_{y\in light(x)}dp_{y,0},\max\limits_{y,z\in light(x)}dp_{y,1}+dp_{z,1}\}$

于是

$dp_{x,1}=\max\{g_{x,1}+dp_{son,1}\}+w(x)$
$dp_{x,0}=\max\{dp_{son,0},g_{x,0},dp_{son,1}+g_{x,1}\}$

定义广义矩阵乘法为 $\max a_{i,k}+b_{k,i}$

就能这样转移
$$
\begin{bmatrix}\begin{array}{c}0 & g_{x,1} & g_{x,0}\\-\infty & w(x) &g_{x,1}+w(x)\\-\infty & -\infty & 0\end{array}\end{bmatrix}\begin{bmatrix}\begin{array}{c}dp_{son,0}\\dp_{son,1}\\0\end{array}\end{bmatrix}=\begin{bmatrix}\begin{array}{c}dp_{x,0}\\dp_{x,1}\\0\end{array}\end{bmatrix}
$$
然后LCT上DDP即可
