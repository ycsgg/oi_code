...
int main() {
    ...
    while (T--) {
        int n;
        cin >> n;
        vector<Polynomial<ll>> f(n, Polynomial<ll>(2));
        for (int i = 0; i < n; i++) {
            ll x;
            cin >> x;
            x %= mod;
        ...
        static function<Polynomial<ll>(int l, int r)> solve =
            [&](int l, int r) -> Polynomial<ll> {
            if (l == r) {
                return f[l];
            }
            if (r - l == 1) {
        ...
        auto &&res = Derivative(Ln(solve(0, n - 1)));
        cout << res << endl;
        ll ans = 0;
        for (int i = 0; i < n; i++) {
            ans ^= (mod - res[i] >= mod ? -res[i] : mod - res[i]);
        }
        cout << ans << endl;
    }
    return 0;
