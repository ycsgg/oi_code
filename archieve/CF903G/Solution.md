...
先转化成最小割处理，因为最小割的性质，我们如果割断了一个 $A_i \rightarrow A_{i+1}$ 的边，那么 $A_j(j >i)$ 的边都不会去割断了

同理如果是割断的 $B_j \rightarrow B_{j+1}$ 则剩下的边也不会被割断，假设选择了边 $A_u,B_v$ 割断，那么代价 $A_u+B_v+\sum\limits_{i<u,j>v} w(i,j)$ 

就是固定的，这个只需要用线段树跑出来最优代价即可，同时可以看到这个与 $A_u$ 无关，所以更新就只更新 $A_u$
