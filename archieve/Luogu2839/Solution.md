...
求中位数考虑二分答案，大于的变成 $1$ ，小于的变成 $-1$ ，和大于 $0$ 说明还可以再大，反之同理

而 $[a,b] \sim [c,d]$ 的询问，最优的肯定是选取 $[a,b]$ 的最大右段和与 $[c,d]$ 的最大左段和

暴力重建显然会 T 飞

考虑对每一个 $mid$ 值建一个线段树，发现 $mid \rightarrow mid+1$ 的过程中只有 $mid$ 的值会变成 $-1$，所以用主席树去维护这个变化继承历史版本
