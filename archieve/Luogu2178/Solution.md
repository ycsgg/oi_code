...
简化题意就是求多少个字串$Len(lcp)\ge r$，至于乘积最大是附赠的

既然是$lcp$就可以考虑$height$数组从大到小排序

因为两杯酒是$r$相似，一定也是$0\dots r-1$相似的，所以大的$lcp$会对小的产生贡献，反之不会

每次都把两个串加进并查集里，根据乘法原理方案就是每个并查集$size$的乘积
