...
线段树的话下放标记复杂度过高，于是采用暴力分块（？）

每个块维护首项和公差，所以块内的每个点都能写成关于下标的一次函数

于是我们考虑一个块里的$(i,v_i)$点斜率为$-d$，首项可以最后加上，对整个块的修改仅需改变斜率

所以传统艺能维护上凸壳，二分查找截距最大的$ans_i$

对块的一部分操作就暴力重构

复杂度大概$O(m\sqrt n\log n)$
