...
void print(int x, int fa) {
    ...
    cout << x << "\t ranging from " << lp[x] << " to " << rp[x] << endl;
    if (isLeft[x]) {
        cout << x << "\t is Left Node" << endl;
    }
    if (isRight[x]) {
        cout << x << "\t is Right Node" << endl;
    }
    cout << x << "\t 's totAns : " << tAns[x] << endl;
    cout << x << "\t 's leftAns : " << lAns[x] << endl;
    cout << x << "\t 's rightAns : " << rAns[x] << endl;
    cout << x << "\t Type : " << typ[x] << endl;
    cout << "*----------------*" << endl;
}
void solve() {
    rAns.resize(cnt + 1);
    ...
    for (int i = 1; i <= cnt; i++) {
        for (int j = 0; j < son[i].size(); j++) {
            ind[son[i][j]] = j;
            // cout << i << "'s son:" << son[i][j] << " index : " << j << endl;
        }
        if ((son[i].size())) {
            isLeft[son[i][0]]      = 1;
