...
看到查询的东西没啥比较好的性质，考虑莫队拆成三个询问然后分别处理每个区间的出现的数字然后求交集

具体来说，因为有相同的数字出现次数也要求交，在离散化的时候相同的数字有着相同的编号，那么相同的数字就表示重复的次数

这个状态可以塞进bitset里，也就是重复$k$次的数字会在bitset里留出$k$个空位用以求交
