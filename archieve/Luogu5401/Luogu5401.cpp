...
int main() {
    ...
    ifac[D] = qpow(fac[D], mod - 2) % mod;
    for (int i = D - 1; i >= 2; i--) {
        ifac[i] = 1ll * ifac[i + 1] * (i + 1) % mod;
    }
    static auto C = [&](const int n, const int m) -> ll {
        if (n < m) {
            return 0;
        }
    ...
    g.resize(D + 1);
    for (int i = 0; i <= D; i++) {
        f[i] = 1ll * qpow((D - 2 * i + mod) % mod, n) % mod * ifac[i] % mod;
        if (i & 1)
            f[i] = (mod - f[i]) % mod;
        g[i] = ifac[i];
    }
    f = f * g;
    f.resize(D + 1);
    auto inv2 = qpow(2, mod - 2);
    for (int i = 0; i <= D; i++) {
        f[i] = 1ll * f[i] * fac[D] % mod * ifac[D - i] % mod *
               qpow(2, mod - 1 - i) % mod;
    }
    reverse(f.a.begin(), f.a.begin() + D + 1);
    g.a.clear();
    g.a.assign(D + 1, 0);
    for (int i = 0; i <= D; i++) {
        f[i] = 1ll * f[i] * fac[D - i] % mod;
    }
    for (int i = 0; i <= D; i++) {
        g[i] = ifac[i];
        if (i & 1)
