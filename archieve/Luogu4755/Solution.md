...
对于区间最大值位置 $pos$，统计所有过 $pos$ 的 $[x,y]$ ，然后递归 $[x,pos-1]$ $[pos+1,y]$ 即可，正确性显然

统计的过程仅需要枚举 $pos$ 的一边，找到 $a_ia_j \le a_{pos}$ 的数量，即 $a_j\le \lfloor\dfrac{a_{pos}}{a_i} \rfloor$ 的数量

这个可以主席树统计，同时用启发式思路枚举较小的一遍就可以做到 $O(n\log^2n)$
